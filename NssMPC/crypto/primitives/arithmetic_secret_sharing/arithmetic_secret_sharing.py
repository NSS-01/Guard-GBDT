...
# from NssMPC.crypto.protocols.arithmetic_secret_sharing.semi_honest_functional import truncABY
class ArithmeticSecretSharing(ArithmeticBase):
    ...
    def __matmul__(self, other):
        if isinstance(other, ArithmeticSecretSharing):
            ...
            res = ArithmeticSecretSharing(cuda_matmul(self.item, other), self.party)
        else:
            raise TypeError(f"unsupported operand type(s) for @ '{type(self)}' and {type(other)}")
    ...
    @classmethod
    def sigmoid(cls,x):
        return (secure_tanh(x)+RingTensor.convert_to_ring(1))*RingTensor.convert_to_ring(0.5)
    @staticmethod
